...
let movementPacketSaver = false;
let playerDataPacketSaver = false;
...
function setConnected(isConnected) {
    ...
    if (isConnected) {
        statusElement.style.color = "#4cff4c"; // bright green
        statusElement.textContent = "[ Connected to Client ]";
    } else {
        statusElement.style.color = "#ff4c4c"; // red
...
function setWaiting() {
    ...
    statusElement.style.color = "#ff9900"; // debug orange
...
let OP_MOVEMENT_PACKET_LENGTH = 143;
let OP_RAYCAST_PACKET_LENGTH = 100;
let OP_PLRHIT_PACKET_LENGTH = 56;
let OP_KILLSTREAK_PACKET_LENGTH = 83;

function getPacketType2(binaryString) {
    if (binaryString.length === OP_MOVEMENT_PACKET_LENGTH) {
        return { length: OP_MOVEMENT_PACKET_LENGTH, code: 12, data: btoa(binaryString), type: "MOVEMENT_DATA" }
    } else if (binaryString.length === OP_RAYCAST_PACKET_LENGTH) {
        return { length: OP_RAYCAST_PACKET_LENGTH, code: 16, data: btoa(binaryString), type: "RAYCAST_DATA" }
    } else if (binaryString.length === OP_PLRHIT_PACKET_LENGTH) {
        return { length: OP_PLRHIT_PACKET_LENGTH, code: 62, data: btoa(binaryString), type: "PLAYER_HIT_DATA" }
    } else if (binaryString.length === OP_KILLSTREAK_PACKET_LENGTH) {
        return { length: OP_KILLSTREAK_PACKET_LENGTH, code: 22, data: btoa(binaryString), type: "KILLSTREAK_DATA" }
    } else {
        return { length: binaryString.length, code: 0, data: btoa(binaryString), type: "UNKNOWN" }
    }
}

function getPacketType(binaryString) {
    let packetType = "UNKNOWN";
    let code = binaryString[2].charCodeAt(0);

    switch (code) {
        case 201:
            // Player Update Data
            packetType = "MOVEMENT_DATA";
            break;
        case 252:
            // Player Data
            packetType = "PLAYER_DATA";
            break;
        case 253:
            // Target Actor Packet
            packetType = "RAYCAST_DATA";
            break
        case 227:
            packetType = "CREATE_GAME";
            break;
    }

    return { length: binaryString.length, code: code, data: btoa(binaryString), type: packetType }
}
...
function grabHealthFromPacket(data) {
    let health;
    Object.values(data.sections[1][1].data[0]).forEach(v => {
        if (typeof(v.data) === "object") {
            health = v.data[15].data;
        }
    });
...
document.getElementById("processPhoton").addEventListener("click", async () => {
    ...
    if (base64) {
        let packet = PhotonPacket.fromBase64(base64);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function createEditorWindow(selectedFile) {
        ...
        editorContainer.style.backgroundColor = "#192734";
        editorContainer.style.border = "1px solid #38444d";
        ...
        editorContainer.style.boxShadow = "0 4px 6px rgba(0, 0, 0, 0.3)";
    
        // Menu bar
        const menuBar = document.createElement("div");
        menuBar.style.backgroundColor = "#15202b";
        menuBar.style.padding = "5px 10px";
        menuBar.style.borderBottom = "1px solid #38444d";
        ...
        menuItems.forEach(menu => {
            ...
            menuItem.style.padding = "2px 8px";
            menuItem.style.transition = "background 0.2s";
            menuItem.onmouseover = () => menuItem.style.backgroundColor = "#253341";
            ...
            dropdown.style.backgroundColor = "#192734";
            dropdown.style.border = "1px solid #38444d";
            dropdown.style.borderRadius = "6px";
            dropdown.style.display = "none";
            dropdown.style.zIndex = "10002";
            dropdown.style.padding = "5px 0";
            dropdown.style.minWidth = "120px";
            dropdown.style.boxShadow = "0 2px 4px rgba(0, 0, 0, 0.3)";
    
            menu.options.forEach(option => {
                const optionItem = document.createElement("div");
                optionItem.textContent = option;
                optionItem.style.padding = "5px 10px";
                optionItem.style.color = "#ffffff";
                optionItem.style.cursor = "pointer";
                optionItem.style.transition = "background 0.2s";
                optionItem.onmouseover = () => optionItem.style.backgroundColor = "#253341";
            ...
            menuItem.onclick = (e) => {
                ...
                dropdown.style.top = `${menuItem.offsetTop + menuItem.offsetHeight}px`;
            };
        ...
        saveButton.onmouseover = () => saveButton.style.backgroundColor = "#1a91da";
        saveButton.onmouseout = () => saveButton.style.backgroundColor = "#1da1f2";
        ...
        switch (menu) {
            case "File":
                if (option === "Save File") {
                    ...
                    const newPath = `DeserializedPackets/new_${path.basename(filePath)}`;
                    saveFile(newPath, editor.value);
                }
                break;
            case "Edit":
                if (option === "Undo") {
                    editor.value = editor.dataset.lastValue || editor.value;
                } else if (option === "Redo") {
                    logToConsole("Redo not fully implemented");
                } else if (option === "Clear File") {
                    editor.dataset.lastValue = editor.value;
                    editor.value = "";
                }
                break;
            case "Tools":
...
function createContextMenu(selectedFile) {
    ...
    menu.style.backgroundColor = "#192734"; /* Twitter dark gray */
    menu.style.border = "1px solid #38444d"; /* Twitter border gray */
    menu.style.padding = "5px 0"; /* Adjusted padding for cleaner look */
    menu.style.zIndex = "10000"; /* Higher z-index to match Twitter's layering */
    menu.style.minWidth = "160px"; /* Slightly wider for better readability */
    menu.style.borderRadius = "6px"; /* Matches Twitter's rounded corners */
    menu.style.boxShadow = "2px 2px 5px rgba(0, 0, 0, 0.3)"; /* Subtle shadow */

    const deleteOption = document.createElement("div");
    deleteOption.classList.add("menu-option", "delete");
    deleteOption.textContent = "Delete";
    deleteOption.style.padding = "10px"; /* Increased padding for touch-friendly design */
    deleteOption.style.cursor = "pointer";
    deleteOption.style.color = "#f4212e"; /* Twitter red for delete */
    deleteOption.style.fontSize = "14px";
    deleteOption.style.transition = "background 0.2s"; /* Smooth hover transition */
    deleteOption.onmouseover = () => deleteOption.style.backgroundColor = "#2e1517"; /* Dark red hover */
    ...
    openLocationOption.textContent = "Open File Location";
    openLocationOption.style.padding = "10px";
    openLocationOption.style.cursor = "pointer";
    openLocationOption.style.color = "#17bf63"; /* Twitter green */
    openLocationOption.style.fontSize = "14px";
    openLocationOption.style.transition = "background 0.2s";
    openLocationOption.onmouseover = () => openLocationOption.style.backgroundColor = "#152e22"; /* Dark green hover */
    ...
    editOption.style.color = "#1da1f2"; /* Twitter blue */
    editOption.style.fontSize = "14px";
    editOption.style.transition = "background 0.2s";
    editOption.onmouseover = () => editOption.style.backgroundColor = "#152e3d"; /* Dark blue hover */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // MITM Checkboxes
    const spinBotCheckbox = document.getElementById("spinBot");
    const pingSpoofCheckbox = document.getElementById("pingSpoof");
    const slideCheckbox = document.getElementById("slide");
    const rank232SpoofCheckbox = document.getElementById("rank232Spoof");
    const rank0SpoofCheckbox = document.getElementById("rank0Spoof");
    const rank255SpoofCheckbox = document.getElementById("rank255Spoof");
    const rank222SpoofCheckbox = document.getElementById("rank222Spoof");
